...
function ShareButtons({ title }: { title: string }) {
  ...
  return (
    <div className={cn('flex', 'items-center', 'gap-2', 'flex-wrap')}>
      <p className={cn('text-[11px]', 'font-inter', 'font-semibold', 'text-[#929296]', 'uppercase', 'tracking-widest', 'w-full', 'mb-[4px]')}>Share</p>
      <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(url)}`}
        target="_blank" rel="noopener noreferrer" aria-label="Share on X"
        className={cn('flex', 'items-center', 'gap-1.5', 'text-[12px]', 'font-inter', 'text-[#555]', 'border', 'border-[#E5E4E0]', 'px-[10px]', 'py-[6px]', 'rounded-full', 'hover:border-black', 'hover:text-black', 'transition-colors', 'duration-200')}>
        <svg width="13" height="13" className='-mt-0.75' viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
        X
      </a>
      <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}`}
        target="_blank" rel="noopener noreferrer" aria-label="Share on LinkedIn"
        className={cn('flex', 'items-center', 'gap-1.5', 'text-[12px]', 'font-inter', 'text-[#555]', 'border', 'border-[#E5E4E0]', 'px-[10px]', 'py-[6px]', 'rounded-full', 'hover:border-black', 'hover:text-black', 'transition-colors', 'duration-200')}>
        <svg width="13" height="13" className='-mt-0.75' viewBox="0 0 24 24" fill="currentColor"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" /><rect x="2" y="9" width="4" height="12" /><circle cx="4" cy="4" r="2" /></svg>
        LinkedIn
      </a>
      <button onClick={copy} aria-label="Copy link"
        className={cn('flex', 'items-center', 'gap-1.5', 'text-[12px]', 'font-inter', 'text-[#555]', 'border', 'border-[#E5E4E0]', 'px-[10px]', 'py-[6px]', 'rounded-full', 'hover:border-black', 'hover:text-black', 'transition-colors', 'duration-200')}>
        <svg width="13" height="13" className='-mt-0.75' viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" /></svg>
    ...
    <div className={cn('relative', 'bg-[#f3f3f3]')}>
      ...
      <section className={cn('max-w-[1440px]', 'mx-auto', 'md:px-[36px]', 'px-[16px]', 'md:py-[96px]', 'py-[64px]')}>
        <div className={cn('flex', 'gap-[48px]', 'items-start')}>
            ...
            {headings.length > 0 && (
              <details className={cn('lg:hidden', 'mb-[32px]', 'border', 'border-[#E5E4E0]', 'rounded-[8px]', 'p-[16px]')}>
                ...
                <nav className="mt-[12px]">
                  <ul className={cn('flex', 'flex-col', 'gap-[8px]')}>
                    {headings.map(h => (
                      <li key={h.id}>
                        <a href={`#${h.id}`} className={cn('text-[13px]', 'font-inter', 'text-[#555]', 'hover:text-black', 'transition-colors', 'duration-200', 'leading-[1.4]')}>{h.text}</a>
                      </li>
                    ))}
                  </ul>
                </nav>
          ...
          <aside className={cn('hidden', 'lg:block', 'w-[300px]', 'flex-shrink-0', 'sticky', 'top-[100px]')}>
            <div className={cn('flex', 'flex-col', 'gap-[32px]')}>
              {headings.length > 0 && (
                <section className={cn('h-[400px]', 'overflow-auto')}>
                  <h2 className={cn('text-[11px]', 'font-inter', 'font-semibold', 'text-[#929296]', 'uppercase', 'tracking-widest', 'mb-[12px]')}>On this page</h2>
              ...
              <section className={cn('relative', 'rounded-[16px]', 'p-[20px]', 'flex', 'flex-col', 'gap-[14px]', 'overflow-hidden')}
              ...
              {others.length > 0 && (
                <section>
                  <h2 className={cn('text-[11px]', 'font-inter', 'font-semibold', 'text-[#929296]', 'uppercase', 'tracking-widest', 'mb-[12px]')}>Related Articles</h2>
                  <ul className={cn('flex', 'flex-col', 'gap-[12px]')}>
                    {others.map(p => {
                      const img = p._embedded?.['wp:featuredmedia']?.[0]?.source_url
                      return (
                        <li key={p.id}>
                          <Link href={`/blog/${p.slug}`} className={cn('flex', 'items-start', 'gap-[10px]', 'group')}>
                            <div className={cn('relative', 'w-[48px]', 'h-[48px]', 'shrink-0', 'rounded-[6px]', 'overflow-hidden', 'bg-[#f2f2f2]')}>
                              {img && <Image src={img} alt={p.title.rendered} fill sizes="48px" className="object-cover" />}
                            </div>
                            <span className={cn('text-[12px]', 'font-inter', 'text-[#555]', 'line-clamp-2', 'group-hover:text-black', 'transition-colors', 'duration-200', 'leading-[1.5]')}
            ...
            <div className={cn('flex', 'items-start', 'gap-[20px]')}>
              {author?.avatar_urls?.['96'] ? (
                <Image src={author.avatar_urls['96']} alt={author.name} width={72} height={72}
                  className={cn('rounded-full', 'object-cover', 'w-[72px]', 'h-[72px]', 'shrink-0', 'ring-4', 'ring-white')} />
              ) : (
                <div className={cn('w-[72px]', 'h-[72px]', 'rounded-full', 'bg-[#111212]', 'flex', 'items-center', 'justify-center', 'text-white', 'font-mont', 'font-bold', 'text-xl', 'shrink-0', 'ring-4', 'ring-white')}>
                  {author?.name?.charAt(0) ?? 'T'}
                </div>
              )}
              <div className={cn('flex', 'flex-col', 'gap-[4px]')}>
                ...
                {author?.link && (
                  <Link target="_blank" rel="noopener noreferrer" href={"/about"}
                    ...
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className='-mt-[3px]'>
      ...
      {/* Continue reading */}
      {others.length > 0 && (
        <section className={cn('max-w-[1440px]', 'mx-auto', 'md:px-[36px]', 'px-[16px]', 'pb-[96px]', 'pt-[56px]')}>
          <h2 className={cn('text-[24px]', 'md:text-[32px]', 'font-mont', 'font-semibold', 'text-black', 'mb-[32px]')}>Continue Reading</h2>
          <div className={cn('grid', 'grid-cols-1', 'sm:grid-cols-2', 'lg:grid-cols-3', 'gap-x-[24px]', 'gap-y-[48px]')}>
            {others.map(p => {
              const img = p._embedded?.['wp:featuredmedia']?.[0]?.source_url
              const t = readingTime(p.content.rendered)
              return (
                <Link key={p.id} href={`/blog/${p.slug}`}>
                  <article className={cn('flex', 'flex-col', 'h-full', 'group', 'cursor-pointer', 'bg-white', 'border', 'border-[#E5E4E0]', 'rounded-[16px]', 'overflow-hidden', 'hover:shadow-xl', 'hover:-translate-y-[4px]', 'hover:border-[#C8C7C3]', 'transition-all', 'duration-300')}>
                    <div className={cn('relative', 'w-full', 'aspect-[3/2]', 'shrink-0', 'overflow-hidden', 'bg-[#f2f2f2]')}>
                      {img && <Image src={img} alt={p.title.rendered} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" className={cn('object-cover', 'transition-transform', 'duration-500', 'group-hover:scale-[1.03]')} />}
                    </div>
                    <div className={cn('flex', 'flex-col', 'gap-[10px]', 'p-[20px]', 'flex-1')}>
                      <span className={cn('flex', 'items-center', 'gap-[5px]', 'text-[12px]', 'font-inter', 'text-[#929296]')}>
                        <Clock size={13} />{t}
                      </span>
                      <h3 className={cn('text-[17px]', 'lg:text-[19px]', 'font-mont', 'font-bold', 'leading-[1.35]', 'text-black', 'group-hover:text-black/70', 'transition-colors', 'duration-300', 'line-clamp-2')}
                        dangerouslySetInnerHTML={{ __html: p.title.rendered }} />
                      <div className={cn('flex', 'items-center', 'justify-between', 'mt-auto', 'pt-[12px]', 'border-t', 'border-[#F0EFED]')}>
                        <span className={cn('text-[11px]', 'font-inter', 'text-[#ABABAB]')}>{format(new Date(p.date), 'MMM d, yyyy')}</span>
                        <div className={cn('flex', 'items-center', 'gap-[6px]', 'bg-black/5', 'group-hover:bg-black', 'group-hover:text-white', 'text-black', 'rounded-full', 'px-[12px]', 'py-[5px]', 'transition-all', 'duration-300')}>
                          <span className={cn('text-[12px]', 'font-mont', 'font-semibold', 'leading-none')}>Read</span>
                          <ArrowRight size={12} className={cn('group-hover:translate-x-[2px]', 'transition-transform', 'duration-300')} />
                        </div>
                      </div>
                    </div>
                  </article>
